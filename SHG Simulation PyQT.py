...
from PyQt6.QtWidgets import (QApplication, QWidget, QLabel, QPushButton, QGridLayout, QSpinBox, QHBoxLayout,
                             QVBoxLayout, QListWidget, QToolBar, QStatusBar, QMenu, QMenuBar, QMainWindow, QSplashScreen)
...
class ContactWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.center()
        self.setWindowTitle('Group Website')
        self.setStyleSheet('background-color:#F2F3F4')
        self.hide()
        qr_label = QLabel(self)
        qr_code = QPixmap("frame.png").scaled(193, 250)
        qr_label.setPixmap(qr_code)
        self.setCentralWidget(qr_label)
        vbox = QVBoxLayout()
...
class Version(QWidget):
    ...
    def initUI(self):
        ...
        self.center()
        self.setWindowTitle('Version ' + current_version)
        self.setStyleSheet('background-color:#F2F3F4')
        ...
        # Resize the image to a new size (200x150 pixels)
        new_size = (240, 120)
        resized_img = img.resize(new_size, Image.LANCZOS)  # or Image.BICUBIC

        # Save the resized image to a file
        resized_img.save("Image/About.png")

        au_logo_Label = QLabel(self)
        # au_logo_Label.setScaledContents(True)
        au_logo = QPixmap("Image/About.png")
        au_logo_Label.setPixmap(au_logo)
        au_logo_Label.setAlignment(Qt.AlignmentFlag.AlignCenter)

        # au_logo_Label.move(72, 0)
        # self.setCentralWidget(au_logo_Label)

        title_font = QFont('Helvetica', 14, 2)
        text_font = QFont('Helvetica', 11)
        title_label = QLabel("SHG Simulation Package")
        title_label.setFont(title_font)
        title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        # title_label.move(64, 10)
        # self.setCentralWidget(title_label)

        ver_label = QLabel("Version " + current_version)
        ver_label.setFont(text_font)
        ver_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        # ver_label.move(64, 20)
        # self.setCentralWidget(ver_label)

        lab_label = QLabel("Auburn University Ultrafast Nonlinear Optics Lab 2023")
        lab_label.setFont(text_font)
        lab_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        # lab_label.move(64, 30)
        # self.setCentralWidget(lab_label)

        rights_label = QLabel("All rights reserved")
        rights_label.setFont(text_font)
        rights_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        # rights_label.move(70, 75)

        web_label = QLabel(self)
        urlLink = "<a href=\"http://wp.auburn.edu/JinLab/\">Github</a>"
        web_label.setText(urlLink)
        rights_label.setFont(text_font)
        web_label.setOpenExternalLinks(True)
        web_label.setAlignment(Qt.AlignmentFlag.AlignCenter)


        layout.addWidget(au_logo_Label)
...
class polarplotGUI(Frame):
    ...
    def __init__(self):
        ...
        self.about_page_window = None
        self.version_page_window = None
        self._menu()
        # self.setGeometry(200,200,500,350)
        self.setFixedSize(500, 350)
        self.center()
        self.setWindowTitle('SHG Simulation V1.0.0')
        self.setWindowIcon(QIcon('Image/auburn.png'))
        self.setStyleSheet('background-color:#F2F3F4')
        ...
        # Function for centering all the window on the screen
    def center(self):
        frame_geometry = self.frameGeometry()
        screen_geometry = QApplication.primaryScreen().geometry()
        x = (screen_geometry.width() - self.width()) // 2
    ...
    def _menu(self):
        ...
        open_action.triggered.connect(self._quit)
        ...
        save_action.triggered.connect(self._quit)
        ...
        # Edit Menu bar: Implement the export feature for graph, expression and character table
        export_graph_action = QAction('&Export Polar Graph...', self)
        export_graph_action.setStatusTip('Export the polar plots')
        export_graph_action.setShortcut('Ctrl+E')
        export_graph_action.triggered.connect(self._quit)
        edit_menu.addAction(export_graph_action)
        # Disable the menu bar selection
        export_graph_action.setEnabled(False)

        export_latex_action = QAction('&Export Expression (Latex)...', self)
        export_latex_action.setStatusTip('Export the Latex Expression')
        # export_graph_action.setShortcut('cmd+E')
        export_latex_action.triggered.connect(self._quit)
        edit_menu.addAction(export_latex_action)
        # Disable the menu bar selection
        export_latex_action.setEnabled(False)

        export_text_action = QAction('&Export Expression (Text)...', self)
        export_text_action.setStatusTip('Export the Text Expression')
        # export_graph_action.setShortcut('cmd+E')
        export_text_action.triggered.connect(self._quit)
        edit_menu.addAction(export_text_action)
        # Disable the menu bar selection
        export_text_action.setEnabled(False)

        export_table_action = QAction('&Export Character Table...', self)
        export_table_action.setStatusTip('Export the Character Table')
        # export_graph_action.setShortcut('cmd+E')
        export_table_action.triggered.connect(self._quit)
        edit_menu.addAction(export_table_action)
        # Disable the menu bar selection
        export_table_action.setEnabled(False)

        # About Menu Bar: This menu bar calls the function of Contact information page
        # Also calls the about us page
        contact_action = QAction('&Contact Us', self)
        contact_action.setStatusTip('Contact Information')
        contact_action.setShortcut('F1')
        contact_action.triggered.connect(self.contact)
        help_menu.addAction(contact_action)

        contact_action = QAction('&Calculation Model', self)
        contact_action.setStatusTip('Equations Used for the model')
        contact_action.setShortcut('F2')
        contact_action.triggered.connect(self.contact)
        help_menu.addAction(contact_action)

        about_action = QAction('&About', self)
        about_action.setStatusTip('About the package')
    ...
    def contact(self, checked):
        if self.contact_window is None:
            self.cw = ContactWindow()
            self.cw.show()
        else:
            self.cw = ContactWindow()
            self.cw.close()  # Close window.
            self.contact_window = None  # Discard reference.
...
if __name__ == '__main__':
    ...
    try:
        ...
        splash_image = QPixmap('Image/Startup.png')
        splash = QSplashScreen(splash_image, Qt.WindowType.WindowStaysOnTopHint)
        splash_font = QFont('Arial', 12)
        current_sys = platform.system()
        current_mach = platform.machine()
        current_platform = platform.platform()
        current_version = '1.0.0'
        splash_label = QLabel(
            'Current Version: ' + current_version + ' Current System: ' + current_sys + ' ' + str(current_platform),
            splash)
        splash_label.setFont(splash_font)
        splash_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        splash_label.move(5, 423)
        splash.show()
        window = polarplotGUI()
        ...
        timer = QTimer()
        timer.singleShot(5000, show_main_window)
